...
# 新增：Shadowrocket 订阅文件路径
SHADOWROCKET_OUT_PATH="${CLASH_WEB_DIR}/shadowrocket_sub"
...
# perform safe substitutions for Clash
TMPF="${CLASH_OUT_PATH}.tmp"
TARGET="${CLASH_OUT_PATH}"
...
# --- B. Shadowrocket 订阅生成 ---
# 原理：将 hystreia2://... 链接本身进行 Base64 编码即可作为订阅内容
# -w 0 防止 Linux base64 命令自动换行（客户端可能解析错误）
echo -n "${URI}" | base64 -w 0 > "${SHADOWROCKET_OUT_PATH}"
echo "[OK] Shadowrocket 订阅已写入：${SHADOWROCKET_OUT_PATH}"
...
server {
    ...
    # Shadowrocket 订阅文件 (Base64 文本)
    location /shadowrocket_sub {
        default_type text/plain;
        try_files /shadowrocket_sub =404;
    }
...
echo
echo "2. Shadowrocket (小火箭) 订阅链接："
echo "   http://${SELECTED_IP}:${HTTP_PORT}/shadowrocket_sub"
echo
echo "提示：请在 Shadowrocket 点击右上角 + 号，类型选择 Subscribe (订阅)，"
echo "      填入上面的第 2 个链接即可自动更新节点。"
echo "========================================="
